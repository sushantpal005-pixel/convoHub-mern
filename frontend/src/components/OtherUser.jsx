            ...
            <div onClick={()=>selectedUserHandler(user)} className={` ${selectedUser?._id === user?._id ? 'flex bg-white/20': ''} flex gap-2 items-center hover:bg-white/20 rounded p-2 cursor-pointer`}>
                <div className='avatar relative'>
                    <div className='w-12 rounded-full '>
                        <img className='' src={user?.profilePhoto} alt="userprofile" />
                        {/* {isOnline? (<span className="absolute top-1 right-0 w-2 h-2 bg-green-500 border-2 border-white rounded-full"></span>): ""}
                         */}
                          {isOnline && (
